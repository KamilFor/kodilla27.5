...
export const loadSeatsRequest = () => {
  return async (dispatch) => {
    ...
    try {
      ...
      await new Promise();
      // SETTIMEOUT (resolve) => setTimeout(resolve, 2000)
...
export default function reducer(statePart = initialState, action = {}) {
  switch (action.type) {
    ...
    case START_REQUEST:
      return {
        ...statePart,
        requests: { ...statePart.requests, [action.payload.name]: { pending: true, error: null, success: false } },
      };
    case END_REQUEST:
      return {
        ...statePart,
        requests: { ...statePart.requests, [action.payload.name]: { pending: false, error: null, success: true } },
      };
    case ERROR_REQUEST:
      return {
        ...statePart,
