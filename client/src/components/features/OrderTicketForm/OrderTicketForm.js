      ...
      <Form className='order-ticket-form' onSubmit={submitForm}>
        <Row>
          <Col xs='12' md='6'>
            {isError && (
              <Alert color='warning'>
                There are some errors in you form. Have you fill all the fields? Maybe you forgot to choose your seat?
              </Alert>
            )}
            {requests['ADD_SEAT'] &&
            requests['ADD_SEAT'].error &&
            !isError && <Alert color='danger'>{requests['ADD_SEAT'].error}</Alert>}
            {requests['ADD_SEAT'] &&
            requests['ADD_SEAT'].success &&
            !isError && (
              <Alert color='success'>You've booked your ticket! Check you email in order to make a payment.</Alert>
            )}
            {requests['ADD_SEAT'] &&
            requests['ADD_SEAT'].pending && <Progress animated className='mb-5' color='primary' value={75} />}
            <FormGroup>
            ...
            <FormGroup check>
              <Label check>
                <Input required type='checkbox' /> I agree with <a href='#'>Terms and conditions</a> and{' '}
          ...
          <Col xs='12' md='6'>
            <SeatChooser chosenDay={order.day} chosenSeat={order.seat} updateSeat={updateSeat} />
          </Col>
        </Row>
      </Form>
    );
